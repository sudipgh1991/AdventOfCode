inputText = `fdaebc egc gc bgefc ecbagd becfa fagc afgbec gdcbfae bgdef | cgfa gcebad ebcadfg cefgbad
...
const newLineStatements = inputText.split('\n');
const statements = [];
newLineStatements.forEach(line => {
    statements.push(line.split('|')[1].trim());
});
let counter = 0;
const calculateNumberOfDigits_Part1 = () => {
    statements.forEach(statement => {
        statement.split(' ').forEach(elem => {
            if ([2, 4, 3, 7].indexOf(elem.length) > -1) {
                counter++;
            }
        })
    });
...
const calculateSumOfDigits_Part2 = () => {
